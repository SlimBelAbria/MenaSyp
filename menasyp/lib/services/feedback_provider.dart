...
// In GoogleSheetApi class add:
static Future<void> deleteRow(int rowIndex) async {
  if (usersheet == null) return;
  final rowLength = await usersheet!.values.row(rowIndex).then((row) => row.length);
  if (rowLength == 0) return;

  await usersheet!.values.insertRow(rowIndex, List.filled(rowLength, ''));
}
